...
class AMBAPI
{
	...
	function getUserCredit($username)
	{
		try
		{
			...
			{
				$response->success=false;
				$response->message='code: '.$httpCode.' message:'.$result['message'];
			}
			return $response;
		}
		catch(Exception $e)
	...
	function transferCreditTo($username, $amount)
	{
		...
		try
		{
			...
			$request = $postData."|||amount:".$amount."|||username:".$username."|||amb_agent:".$GLOBALS['amb_agent'];
		...
		try
		{
			...
			else if($gamemode == "4")
			{
				if($company == "sportsbo")
				{
					...
					$sub_response = new stdClass();
					$sub_response->url = $GLOBALS['amb_api_endpoint']."/#!/redirect?username=".$username."&password=".$password."&url=".$website."&hash=".$GLOBALS['amb_hash']."&state=sport&lang=th";

					$response->data=$sub_response;
					return $response;
				}
				else if($company == "sportsboambbet")
				{
					...
					$sub_response = new stdClass();
					$sub_response->url = "https://aquabet.co/login/auto/?username=".$username."&password=".$password."&url=".$website."&hash=".$GLOBALS['amb_hash']."&state=sport&lang=th";
			...
			if($company == "ag")
			{
				$url = $GLOBALS['amb_api_endpoint']."/v0.1/partner/member/login/track/aggame/".$GLOBALS['amb_key']."/".$username;
			}
			else
			{
	...
	function WithdrawalAG($username, $password)
	{
		try
		{
			$fields = [
			...
			$postData = json_encode($fields);
			$url = $GLOBALS['amb_api_endpoint']."/v0.1/partner/member/logout/track/aggame/".$GLOBALS['amb_key']."/".$username;
